...
First the functionality is, I should be able to schedule my fb posting. So, I made a recipe in IFTTT " IF i recieve a mail from some email address and then post the email to facebook account".
...
h3. And the second app(FB bot)

FB Bot which post the top HN links after few hours
First i tried to scrape the HN site , but godbless my github search, I found the code for it already.

Now I'm using the previous IFTTT recipe and i sending an email.

The reason I'm using mailgun, is sending the email is very simple:
<pre><code>
from conf import *
import requests
...
<button href="https://github.com/Prithvirajbilla/bufferapplike"> Github link </button>
